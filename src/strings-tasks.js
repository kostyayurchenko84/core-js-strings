...
function repeatString(str, times) {
  if (str === undefined || str === null || str.length === 0 || times <= 0)
    return '';
...
function isPalindrome(str) {
  const originalStr = str.replace(/[^a-z]/gi, '').toLowerCase();
  const reverseStr = originalStr.split('').reverse().join('');
  if (originalStr === reverseStr) return true;
  return false;
}
...
function extractNameFromTemplate(value) {
  return value.substring('Hello, '.length, value.length - '!'.length);
}
